import base64, hashlib, json, os, pathlib, secrets, shutil, socket, subprocess, tempfile, time, urllib.request, urllib.error
root = pathlib.Path.cwd()
evidence = root / 'reviews/evidence/storage-improvements'
...
try:
    ...
    seed=api('POST',path+'/commits',worker,{'expected_head':'','files':[{'path':'brief','content':'keep'},{'path':'report','content':'draft'},{'path':'large','content':base64.b64encode(secrets.token_bytes(500000)).decode()}]},key='seed',want=201)
    ...
    shutil.rmtree(scratch/'cache')
    server=launch()
    check('cold content read',request('GET',path+'/file?path=brief',worker)==(200,b'keep'))
    check('content read avoids full Git cache',not list((scratch/'cache').rglob('HEAD')))
    ...
    wal=api('GET',path+'/wal')
    check('incremental Git pack is small',wal[1]['size']<wal[0]['size']//10)
    check('incremental REST pack is small',wal[2]['size']<wal[0]['size']//10)
    (evidence/'publication-costs.json').write_text(json.dumps({'seed_pack_bytes':wal[0]['size'],'git_edit_pack_bytes':wal[1]['size'],'rest_edit_pack_bytes':wal[2]['size']},indent=2)+'\n')
    first=api('GET',path+'/events?after=0&limit=1',read['plaintext'])
    check('event feed starts with committed seed',first['next_after']==1 and first['events'][0]['updates'][0]['new_sha']==seed['sha'])
    rest=api('GET',path+'/events?after=1',read['plaintext'])
    check('events survive restart without retry duplicates',[e['sequence'] for e in rest['events']]==[2,3])
    git('-C',str(clone),'tag','v1')
    git('-C',str(clone),'push','--atomic','origin','main:refs/heads/review','refs/tags/v1')
    page=api('GET',path+'/events?after=3&limit=1',worker)
    check('atomic multi-ref push is one whole event',len(page['events'])==1 and len(page['events'][0]['updates'])==2)
    git('-C',str(clone),'push','origin',':refs/heads/review')
    deleted=api('GET',path+'/events?after=4',worker)
    check('ref-only deletion is durable',len(deleted['events'])==1 and deleted['events'][0]['updates'][0]['new_sha']=='')
    command([str(scratch/'artifacts'),'compact','--account','local','--namespace','interface-check','--repo',name],env=env)
    check('compaction creates no event',api('GET',path+'/events?after=5',worker)['events']==[])
    server.terminate();server.wait(timeout=10);server=None
    shutil.rmtree(scratch/'cache')
    server=launch()
    check('cold checkpoint read',request('GET',path+'/file?path=report',worker)==(200,b'done'))
    git('-C',str(clone),'fetch')
    check('event replay survives compaction',len(api('GET',path+'/events',worker)['events'])==5)
    for i in range(32):
        api('POST',path+'/commits',worker,{'files':[{'path':'counter','content':str(i)}]},want=201)
    server.terminate();server.wait(timeout=10);server=None
    server=launch()
    for _ in range(200):
        checkpoint=command(['docker','exec',container,'psql','-U','artifacts','-d','artifacts','-Atc',"SELECT sequence FROM checkpoints WHERE repo_id='"+created['id']+"'"])
        if checkpoint=='37': break
        time.sleep(.1)
    check('background compaction discovers backlog on restart',checkpoint=='37')
    check('background compaction preserves events',len(api('GET',path+'/events?after=5',worker)['events'])==32)
    ...
    (evidence/'adoption-checks.json').write_text(json.dumps({'passed':True,'checks':checks},indent=2)+'\n')
