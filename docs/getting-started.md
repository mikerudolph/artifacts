...
The repository is empty until you publish its first files. The returned remote is the Git address you will use later.
...
The response includes `result.sha`: the Git commit SHA for this version. REST writes update the paths you supply and preserve other files.

## Read the published files

Read the report at `main`, the current branch:
...
File reads return the original bytes.
...
- [Integrate into your app](/artifacts/core/integration/) connects the API to your backend and workers.
